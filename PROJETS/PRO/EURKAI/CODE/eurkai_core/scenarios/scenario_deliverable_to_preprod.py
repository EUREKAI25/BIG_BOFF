...
def _write_code(deliverable, path):
    """Écrit TOUS les modules Python : entry point + backend + frontend + storage."""
    files             = []
    ...
    # ── Modules additionnels (backend + frontend + storage) ───────────────────
    modules = deliverable.get("modules") or {}
    for layer_key in ("backend", "frontend", "storage"):
        for mod in modules.get(layer_key) or []:
            mod_filename = mod.get("filename") or ""
            mod_code     = mod.get("code", "")
            if not mod_filename or not mod_code:
                continue
            if not mod_filename.endswith(".py"):
                mod_filename += ".py"
            if mod_filename in written_filenames:
                continue
            fp = path / mod_filename
            fp.write_text(mod_code, encoding="utf-8")
            files.append({"name": mod_filename, "path": str(fp), "size": len(mod_code)})
            written_filenames.add(mod_filename)
    ...
    readme.write_text(
        ...
        f"## Entry point\n`{filename}`\n\n"
        f"## Modules générés ({len(py_files)})\n"
