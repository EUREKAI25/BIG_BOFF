...
def render_page(page: Page, extra_head: str = "", extra_body_end: str = "") -> str:
    """Génère le HTML complet d'une page (Eurkai DOM Library)."""
...
def render_section(section: Section) -> str:
    """
    Rendu d'une section selon les règles Eurkai DOM Library :

      NavBarBlock  → aucun wrapper  (full-bleed natif)
      FooterBlock  → aucun wrapper  (full-bleed, layout interne)
      HeroBlock    → <section id>   sans class ni container (full-bleed)
      CTABlock     → <section id>   sans container (CTA gère son propre padding)
      Blocs v0.2 avec container interne (Stat/Steps/FAQ/Pricing/Testimonial/Content)
                   → <section class="section"> sans container supplémentaire
      Modules v0.1 legacy → <section class="section"> + <div class="container">
    """
    ...
    if len(section.columns) == 1 and section.columns[0].span == 12:
        mod   = section.columns[0].module
        inner = render_module(mod)

        if isinstance(mod, NavBarBlock):
            return inner

        if isinstance(mod, FooterBlock):
            return inner

        if isinstance(mod, (HeroBlock, CTABlock)):
            return f'<section{section_id}{bg_style}>\n{inner}\n</section>'

        if isinstance(mod, (StatBlock, StepsBlock, FAQBlock, PricingBlock,
                             TestimonialBlock, ContentBlock)):
            return f'<section{section_id} class="section"{bg_style}>\n{inner}\n</section>'

        # v0.1 legacy → section + container
        return (
            f'<section{section_id} class="section"{bg_style}>\n'
