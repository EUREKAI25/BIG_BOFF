...
def _fallback_variables(theme: dict) -> str:
    ...
    # Surcharges directes depuis assembler (palette pipeline > valeurs génériques)
    _p = theme.get("_palette", {})
    if _p.get("text"):       color_text       = _p["text"]
    if _p.get("text_light"): color_text_light = _p["text_light"]
    if _p.get("bg"):         color_bg         = _p["bg"]
    if _p.get("bg_gray"):    color_bg_subtle  = _p["bg_gray"]
    if _p.get("border"):     color_border     = _p["border"]
