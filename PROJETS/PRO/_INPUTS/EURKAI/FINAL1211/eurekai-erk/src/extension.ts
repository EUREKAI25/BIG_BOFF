...
const store = {
  objects: new Map<string, ErkObject>(),
  vectors: new Map<string, ErkObject>(),
  rules: [] as ErkObject[]
};

let cockpitPanel: vscode.WebviewPanel | undefined;
let fractaleProvider: FractaleProvider;
...
function scanFolder(folderPath: string, depth: number = 0): void {
...
function parseFileFromPath(filePath: string): void {
  try {
...
function parseContent(text: string, source: string): void {
  const lines = text.split('\n');
  let currentObject: ErkObject | null = null;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lineageMatch) {
      if (currentObject) addObject(currentObject);
...
function getChildren(lineage: string): string[] {
  const prefix = lineage + ':';
  return Array.from(store.objects.keys()).filter(l => 
    l.startsWith(prefix) && !l.substring(prefix.length).includes(':')
  );
}

function resolveVector(name: string): any {
  const v = store.vectors.get(name);
  if (!v) return null;
  const def = v.attributes.default;
  if (def?.match(/^\d+(\.\d+)?$/)) return parseFloat(def);
...
class FractaleProvider implements vscode.TreeDataProvider<string> {
  ...
  getTreeItem(lineage: string): vscode.TreeItem {
    ...
    // Icons
    if (lineage.includes(':Entity:')) item.iconPath = new vscode.ThemeIcon('person');
    else if (lineage.includes(':Vector:')) item.iconPath = new vscode.ThemeIcon('symbol-variable');
    else if (lineage.includes(':Rule:')) item.iconPath = new vscode.ThemeIcon('checklist');
    else if (lineage.includes(':Config:')) item.iconPath = new vscode.ThemeIcon('settings-gear');
    else item.iconPath = new vscode.ThemeIcon('symbol-class');
    
    return item;
  }
  
  getChildren(lineage?: string): string[] {
    if (!lineage) return store.objects.has('Object') ? ['Object'] : [];
    return getChildren(lineage);
  }
}

class RulesProvider implements vscode.TreeDataProvider<ErkObject> {
  private _onDidChange = new vscode.EventEmitter<ErkObject | undefined>();
  readonly onDidChangeTreeData = this._onDidChange.event;
  
  refresh() { this._onDidChange.fire(undefined); }
  
  getTreeItem(rule: ErkObject): vscode.TreeItem {
...
function showCockpit(context: vscode.ExtensionContext) {
  if (cockpitPanel) {
    cockpitPanel.reveal();
    updateCockpit();
    return;
...
async function createObject(parent: string, name: string, attributes: Record<string, string>, methods: string[] = [], relations: Array<{type: string, target: string}> = [], rules: Array<{type: string, name: string}> = []) {
...
function showRestartMessage() {
  vscode.window.showInformationMessage(
    'Extension EUREKAI mise à jour. Redémarrez VSCode (Cmd+Q / Ctrl+Q) pour appliquer.',
    'OK'
  );
}

async function deleteObject(lineage: string) {
  const obj = store.objects.get(lineage);
  if (!obj || !obj.source) {
...
async function updateAttribute(lineage: string, key: string, value: string) {
  const obj = store.objects.get(lineage);
...
async function addAttribute(lineage: string, key: string, value: string) {
  const obj = store.objects.get(lineage);
  if (!obj || !obj.source) return;
...
async function addMethod(lineage: string, name: string) {
  const obj = store.objects.get(lineage);
  if (!obj || !obj.source) return;
...
async function addRelation(lineage: string, type: string, target: string) {
  const obj = store.objects.get(lineage);
  if (!obj || !obj.source) return;
...
async function addRule(lineage: string, type: string, name: string) {
  const obj = store.objects.get(lineage);
  if (!obj || !obj.source) return;
...
async function moveObject(sourceLineage: string, targetLineage: string) {
...
function buildTreeText(lineage: string, indent: number, maxDepth: number): string {
...
function getCockpitHtml(objects: any[], vectors: any[], rules: any[], sourceFiles: string[] = [], projectName: string = 'Sans projet'): string {
          ...
          <div class="create-grid">
            <div class="form-group">
              ...
              <select id="createParent" class="form-input">
                ...
                ${Array.from(new Set(objects.map((o: any) => o.lineage))).map((l: any) => 
                  `<option value="${l}">${l.split(':').pop()} (${l})</option>`
                ).join('')}
              </select>
            </div>
      ...
      <div class="tab-panel" id="panel-gevr">
        <div class="explorer" style="max-width: 900px;">
          ...
          <datalist id="methodsList">
            ${objects.filter((o: any) => o.lineage.includes(':Method:')).map((o: any) => 
              `<option value="Method:${o.name}">`
            ).join('')}
          </datalist>
...
function buildTreeData(): any {
  function buildNode(lineage: string): any {
    const obj = store.objects.get(lineage);
    const children = getChildren(lineage);
    return {
      lineage,
...
const vectorDefinitions = new Map<string, {source: string, line: number}>();

function isManifestOrInstance(fileName: string): boolean {
  return fileName.endsWith('.m.gev') || fileName.endsWith('.i.gev');
}

function isSeedOrSchema(fileName: string): boolean {
  return fileName.endsWith('.s.gev') || fileName.endsWith('.t.gev') || 
         fileName.endsWith('.gev') && !isManifestOrInstance(fileName);
}

function validateDocument(document: vscode.TextDocument): void {
  if (!document.fileName.endsWith('.gev') && !document.fileName.endsWith('.erk')) return;
  ...
  const definedLineages = new Set<string>();
  const usedVectors = new Set<string>();
  const localVectorDefs = new Map<string, number>(); // V_xxx -> line number
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lineageMatch) {
      ...
      if (definedLineages.has(currentLineage)) {
        diagnostics.push(new vscode.Diagnostic(
          ...
          `Duplicate lineage: ${currentLineage}`,
    ...
    if (attrMatch) {
      ...
      if (!attrValue.trim()) {
        diagnostics.push(new vscode.Diagnostic(
          new vscode.Range(i, 0, i, line.length),
          `Empty value for attribute: ${attrName}`,
      ...
      if (vectorDefMatch) {
        ...
        if (isManifest) {
          diagnostics.push(new vscode.Diagnostic(
            new vscode.Range(i, line.indexOf(vectorName), i, line.indexOf(vectorName) + vectorName.length),
            `Cannot define vector in manifest/instance. Use existing vector or define in seed.`,
            vscode.DiagnosticSeverity.Error
          ));
        } else {
          ...
          if (existing && existing.source !== fileName) {
            diagnostics.push(new vscode.Diagnostic(
              new vscode.Range(i, line.indexOf(vectorName), i, line.indexOf(vectorName) + vectorName.length),
              `Duplicate vector definition: ${vectorName} (already defined in ${path.basename(existing.source)}:${existing.line + 1})`,
              vscode.DiagnosticSeverity.Error
            ));
          } else if (localVectorDefs.has(vectorName)) {
            diagnostics.push(new vscode.Diagnostic(
              new vscode.Range(i, line.indexOf(vectorName), i, line.indexOf(vectorName) + vectorName.length),
              `Duplicate vector definition: ${vectorName} (already defined at line ${localVectorDefs.get(vectorName)! + 1})`,
              vscode.DiagnosticSeverity.Error
            ));
          } else {
    ...
    if (trimmed && !trimmed.match(/^[A-Z]/) && !trimmed.startsWith('.')) {
      diagnostics.push(new vscode.Diagnostic(
        new vscode.Range(i, 0, i, line.length),
        `Invalid syntax: expected lineage (Name:) or attribute (.name = value)`,
        vscode.DiagnosticSeverity.Error
      ));
    }
  }
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (idx >= 0) {
          if (isManifest) {
            ...
            diagnostics.push(new vscode.Diagnostic(
              new vscode.Range(i, idx, i, idx + vectorName.length),
              `Vector not defined: ${vectorName}. In manifest/instance, vectors must be pre-defined.`,
              vscode.DiagnosticSeverity.Error
            ));
          } else {
            ...
            diagnostics.push(new vscode.Diagnostic(
              new vscode.Range(i, idx, i, idx + vectorName.length),
              `Vector not defined: ${vectorName}. Use V_xxx(type, default, description) to define it.`,
...
function checkUnusedVectors(): void {
...
class ErkDefinitionProvider implements vscode.DefinitionProvider {
  provideDefinition(
    document: vscode.TextDocument,
    position: vscode.Position
    ...
    if (word.startsWith('V_')) {
      ...
      if (vectorObj && vectorObj.source && vectorObj.line > 0) {
        return new vscode.Location(
          vscode.Uri.file(vectorObj.source),
          new vscode.Position(vectorObj.line - 1, 0)
        );
      }
      ...
      for (const [lineage, obj] of store.objects) {
        if (lineage.endsWith(`:${word}`) || obj.name === word) {
          if (obj.source && obj.line > 0) {
            return new vscode.Location(
              vscode.Uri.file(obj.source),
              new vscode.Position(obj.line - 1, 0)
            );
          }
        }
      }
...
class ErkHoverProvider implements vscode.HoverProvider {
  provideHover(
    document: vscode.TextDocument,
    position: vscode.Position
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.languages.registerCompletionItemProvider(selector, new ErkCompletionProvider(), '.', ':', '='),
    vscode.languages.registerDefinitionProvider(selector, new ErkDefinitionProvider()),
    vscode.languages.registerHoverProvider(selector, new ErkHoverProvider())
  );
  
  // Commands
  context.subscriptions.push(
    vscode.commands.registerCommand('eurekai.showCockpit', () => showCockpit(context)),
    vscode.commands.registerCommand('eurekai.refresh', () => {
      parseAllFiles();
      fractaleProvider.refresh();
      rulesProvider.refresh();
      updateCockpit();
    ...
    vscode.commands.registerCommand('eurekai.selectObject', (lineage: string) => {
      const obj = store.objects.get(lineage);
      if (obj && obj.line > 0) {
        vscode.workspace.openTextDocument(obj.source).then(doc => {
          vscode.window.showTextDocument(doc, vscode.ViewColumn.One).then(editor => {
