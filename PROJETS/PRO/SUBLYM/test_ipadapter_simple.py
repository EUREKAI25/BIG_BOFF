"""
TEST IP-ADAPTER FACE ID SIMPLE — Le bon modèle pour cohérence faciale
"""
...
ENDPOINT = "https://queue.fal.run/fal-ai/ip-adapter-face-id"
...
# PROMPT SIMPLE
PROMPT = "A photo of a man standing on a beach in the Maldives. He wears a white shirt and beige shorts. Photo-realistic, cinematic quality, professional photography. Face clearly visible, frontal view."

# OUTPUT
OUTPUT_FILE = "test_ipadapter_simple_output.png"

def main():
    print("=" * 70)
    print("TEST IP-ADAPTER FACE ID — Cohérence faciale")
    print("=" * 70)
    ...
    face_image_url = f"data:image/png;base64,{image_b64}"
    print(f"   Base64: {len(image_b64)} chars")

    # 2. Construire le payload (IP-Adapter Face ID)
    payload = {
        "prompt": PROMPT,
        "face_image_url": face_image_url,  # L'image du visage à reproduire
        "num_images": 1,
    ...
    debug_payload["face_image_url"] = f"<base64 {len(image_b64)} chars>"
    print(json.dumps(debug_payload, indent=2))

    # 3. Appel API
    print(f"\n🚀 Appel API Fal.ai IP-Adapter Face ID...")
    ...
    while elapsed < max_wait:
        ...
        if status == "COMPLETED":
            ...
            # IP-Adapter retourne "image" au singulier
            image_data = result.get("image")
            if not image_data:
                print(f"\n❌ Pas d'image dans le résultat")
                return
