"""
IMAGE GENERATOR GEMINI — Génération images depuis scénario v8
Utilise Gemini 3 Pro (Nano Banana) via Fal.ai pour cohérence maximale
"""
...
PRICING_GEMINI_PRO = 0.04  # USD per image (Fal.ai - Gemini 2.5 Flash)
DEFAULT_MODEL_ENDPOINT = "https://queue.fal.run/fal-ai/gemini-3-pro-image-preview"

# =============================================================================
# IMAGE GENERATOR GEMINI
# =============================================================================

class ImageGeneratorGemini:
    def __init__(self, scenario_path: str, output_dir: str = "generated_images", reference_images: list = None, model_endpoint: str = None):
        self.scenario_path = scenario_path
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)

        # Endpoint configurable
        self.model_endpoint = model_endpoint or os.getenv("GEMINI_ENDPOINT", DEFAULT_MODEL_ENDPOINT)
        ...
        if reference_images:
            if isinstance(reference_images, str):
                ...
                if ref_path.is_dir():
                    ...
                    print(f"🔍 DEBUG: {len(self.reference_images)} images trouvées dans {reference_images}")
    ...
    def generate_image(self, scene_id: int, keyframe_type: str) -> dict:
        """Génère une image via Gemini 3 Pro (Fal.ai)."""

        print(f"\n🎨 Génération image scène {scene_id} ({keyframe_type})...")

        positive_prompt = self._build_positive_prompt(scene_id, keyframe_type)

        # Ajouter instruction de référence si image fournie
        if self.reference_images:
            positive_prompt = f"Based on the reference image provided, use the exact facial features, skin tone, and appearance of this person. {positive_prompt}"

        print(f"   Prompt: {positive_prompt[:100]}...")

        try:
            # Encoder TOUTES les images de référence en base64 si fournies (jusqu'à 14)
            image_urls_data = []
            if self.reference_images:
                print(f"   📸 Encodage de {len(self.reference_images)} images de référence...")
                for i, ref_img in enumerate(self.reference_images):
                    print(f"   🔍 DEBUG [{i+1}/{len(self.reference_images)}]: {Path(ref_img).name}")

                    with open(ref_img, "rb") as img_file:
                        img_bytes = img_file.read()
                        image_b64 = base64.b64encode(img_bytes).decode('utf-8')
                        image_urls_data.append(f"data:image/png;base64,{image_b64}")
                        print(f"       → {len(img_bytes)} bytes, {len(image_b64)} chars en base64")

                print(f"   ✅ {len(image_urls_data)} images encodées et prêtes")

            # Préparer input pour Gemini 3 Pro via Fal.ai
            payload = {
            ...
            # Ajouter images de référence si fournies (Gemini 3 Pro utilise image_urls)
            if image_urls_data:
                payload["image_urls"] = image_urls_data
                print(f"   🔍 DEBUG: image_urls ajouté au payload ({len(image_urls_data)} images)")
                # Debug payload structure
                debug_payload = payload.copy()
                if "image_urls" in debug_payload:
                    debug_payload["image_urls"] = [f"<image {i+1} base64>" for i in range(len(image_urls_data))]
                print(f"   🔍 DEBUG PAYLOAD: {json.dumps(debug_payload, indent=2)}")

            # Appel Fal.ai Gemini 3 Pro (endpoint configurable)
    ...
    def generate_with_validation(self, scene_id: int, keyframe_type: str, max_attempts: int = 3) -> dict:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if attempt < max_attempts:
                ...
                # NE PAS SUPPRIMER pour test/debug
                # Path(result['filepath']).unlink(missing_ok=True)
            else:
                # Dernière tentative échouée → GARDER pour analyse
                print(f"   ⚠️  ÉCHEC après {max_attempts} tentatives → IMAGE CONSERVÉE POUR ANALYSE")
                # Path(result['filepath']).unlink(missing_ok=True)
                result['validation'] = validation
                return result  # Retourner quand même pour analyse
    ...
    def generate_scenes(self, scene_ids: list, keyframes: str = "start", validate: bool = True, max_attempts: int = 3) -> list:
        ...
        print(f"\n🎬 GÉNÉRATION IMAGES — Gemini 3 Pro")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Génération images Gemini depuis scénario v8")
    ...
    parser.add_argument("--output", default="generated_images_gemini",
                        help="Dossier de sortie (défaut: generated_images_gemini)")
    ...
    generator = ImageGeneratorGemini(args.scenario, args.output, args.reference_images)
